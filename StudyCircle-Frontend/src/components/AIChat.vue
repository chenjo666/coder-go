    ...
    messageVOList: [
        {
            ...
            role: 'user',
            content: 'hello',
            sendTime: '2020-01-02 1:01:02'
        },
        {
            messageId: '124124',
            role: 'user',
            content: 'hello',
            sendTime: '2020-01-02 1:02:02'
        },
        {
            messageId: '12312312',
            role: 'user',
            content: '恰噶',
            sendTime: '2020-01-02 1:03:02'
        },
...
const clickConversationEvent = (index) => {
    ...
    findMessageListRequest(chatCompletionVO.value.conversationListVO[index].conversationId)
        .then(res => {
            if (res != null) {
                chatCompletionVO.value.messageVOList = res
                autoScrolToBottom()
            }
        }).catch(err => {
            console.log(err)
            ...
            if (res) {
                ...
                if (index === 0) {
                    if (chatCompletionVO.value.conversationListVO.length === 0) {
                        chatCompletionVO.value.messageVOList = []
                    } else {
...
const clickSendMessageEvent = (index) => {
    ...
    const length = chatCompletionVO.value.messageVOList.length;
    const messageTargetId = length == 0 ? '0' : chatCompletionVO.value.messageVOList[length - 1].messageId;
    ...
    // 自身对话
    chatCompletionVO.value.messageVOList.push(userMessageVO);
    // 2. ai 对象
    const aiMessageVO: MessageVO = {
        messageId: '',
        role: 'assistant',
        content: '',
        sendTime: formatDate(addRandomSeconds(new Date(), 1, 3))
    }
    chatCompletionVO.value.messageVOList.push(aiMessageVO);

    // 3 禁用点击
    btnSendDisabled.value = true

    // 4 加载界面
    const messageLength = chatCompletionVO.value.messageVOList.length;
    ...
    addMessageRequest(conversationId, messageTargetId, messageContent.value)
        .then(res => {
            ...
            if (res !== null) {
                ...
                chatCompletionVO.value.messageVOList[messageLength - 2].messageId = userMessageId

                // 更新 ai 对象的结果
                const aiMessageId = res[1].messageId
                chatCompletionVO.value.messageVOList[messageLength - 1].messageId = aiMessageId
...
const deleteMessageEvent = (index) => {
    delMessageRequest(chatCompletionVO.value.messageVOList[index].messageId)
    .then(response => {
        if (response) {
            chatCompletionVO.value.messageVOList.splice(index, 1);
        }
    }).catch(error => {
        console.log(error)
    })
...
const updateMessageContentEvent = (index) => {
    // 1. 清空记录
    chatCompletionVO.value.messageVOList[index].content = ''
    // 2. 禁用点击
    btnSendDisabled.value = true
    // 3. 加载
    loadingVisible.value = true;

    setMessageContentRequest(chatCompletionVO.value.messageVOList[index].messageId)
    .then(res => {
        // 3. 展示结果
        if (res !== null) {
                // 收到结果后，更新 messageID
                const assistantMessageId = res.messageId
                chatCompletionVO.value.messageVOList[index].messageId = assistantMessageId
...
const updateConversationNameRequest = (conversationId, newConversationName) => {
    return axios.put(`/conversations/${conversationId}`, { params: { 'newConversationName': newConversationName } })
        .then(response => {
            if (response.status !== 200) {
                errorMsg('网络请求出错!')
                return false
...
const addMessageRequest = (conversationId, messageTargetId, question) => {
    return axios.post(`/${conversationId}/messages`, { "messageTargetId" : messageTargetId, 'question': question })
        .then(response => {
            if (response.status !== 200) {
                errorMsg('请求失败！')
                return null
                        ...
                        <div class="chat-detail-container">
                            <el-scrollbar ref="messageScrollbarRef" height="75vh" @scroll="chatItemScroll">
                                <div class="chat-container" ref="messageInnerRef">
                                    <div v-for="(message, index) in chatCompletionVO.messageVOList" :key="index">
                                        <!-- 对方 -->
                                        <div class="chat-word-other" v-if="message.role == 'assistant'">
                                            ...
                                            <div class="chat-info">
                                                <p class="chat-time">{{ message.sendTime }}</p>
                                                <div class="chat-content">
                                                    <el-button loading text class="btn-loading" bg
                                                        v-if="loadingVisible && index === chatCompletionVO.messageVOList.length - 1"></el-button>
